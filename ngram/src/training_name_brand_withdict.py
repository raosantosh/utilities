...
dictionry_file = cwd+'datasets/top_keywords_macys.csv'
...
nb_tokens_query_product_buckets = 2**10
...
def getTriplets(query, length, tokens_set):
    triplets = Set()
    wordSet = Set()
    ...
    return triplets, wordSet


def query2producttripletrepresentation(query, product, BUCKETS, tokens_set, BUCKET_WORD):
    features = Set()
    features_token = Set()
    qgrams_4, tokens_q = getTriplets(query, 3, tokens_set)
    pgrams_4, tokens_p = getTriplets(product, 3, tokens_set)
    for gram in qgrams_4:
        if gram in pgrams_4:
            features.add(abs(int(mmh3.hash(gram))) % BUCKETS)
    for token in tokens_q:
        if token in tokens_p:
            features_token.add(abs(int(mmh3.hash(token))) % BUCKET_WORD)
    return features,features_token
...
###
def next_batch(positives_count, test_negative_samples_factor, fpp, fnn, queries_set):
    ...
    query_productname_tokens = np.zeros((test_data_size, nb_tokens_query_product_buckets, 1))
    query_brand_tokens = np.zeros((test_data_size, nb_tokens_query_product_buckets, 1))
    query_productname_brand_tokens = np.zeros((test_data_size, nb_tokens_query_product_buckets, 1))
    product_index = 0
    ...
    for index in range(positives_count):
        ...
        features_productname_triplets, features_productname_tokens = query2producttripletrepresentation(querynormalized, productnamenormalized, nb_triplets_query_product_buckets,queries_set,nb_tokens_query_product_buckets)
        features_productbrand_triplets,  features_productbrand_tokens = query2producttripletrepresentation(querynormalized, productbrandnormalized, nb_triplets_query_product_buckets,queries_set,nb_tokens_query_product_buckets)
        features_productname_brand_triplets  = features_productname_triplets.intersection(features_productbrand_triplets)
        features_productname_brand_tokens  = features_productname_tokens.intersection(features_productbrand_tokens)
        ...
        for cidx in features_productname_tokens:
            query_productname_tokens[product_index, cidx, 0] = 1
        for cidx in features_productbrand_tokens:
            query_brand_tokens[product_index, cidx, 0] = 1
        for cidx in features_productname_brand_tokens:
            query_productname_brand_tokens[product_index, cidx, 0] = 1
        ...
        while (negatives != test_negative_samples_factor):
            ...
            features_productname_triplets, features_productname_tokens = query2producttripletrepresentation(querynormalized, productnamenormalized,
                                                                                nb_triplets_query_product_buckets,
                                                                                queries_set, nb_tokens_query_product_buckets)
            features_productbrand_triplets, features_productbrand_tokens = query2producttripletrepresentation(querynormalized, productbrandnormalized,
                                                                                nb_triplets_query_product_buckets,
                                                                                queries_set,
                                                                                nb_tokens_query_product_buckets)

            features_productname_brand_triplets = features_productname_triplets.intersection(
                features_productbrand_triplets)
            features_productname_brand_tokens = features_productname_tokens.intersection(
                features_productbrand_tokens)
            for cidx in features_productname_triplets:
                query_productname_triplets[product_index, cidx, 0] = 1
            for cidx in features_productbrand_triplets:
                query_brand_triplets[product_index, cidx, 0] = 1
    ...
    return test_labels, query_productname_triplets,query_brand_triplets,query_productname_brand_triplets,\
           query_productname_tokens, query_brand_tokens, query_productname_brand_tokens

#
def get_next_annotation_data(annotation_file_name, queries_set):
    ...
    for index in range(test_data_size):
        ...
        product_index += 1
    return annotations, in_memo, query_productname_triplets, query_brand_triplets, query_productname_brand_triplets,query_productname_tokens, query_brand_tokens, query_productname_brand_tokens
##
...
with tf.device('/cpu:0'):
    ...
    query_productname_tokens_emb = tf.placeholder(tf.float32, [None, nb_tokens_query_product_buckets, 1],
                                                    name="query_productname_tokens")
    query_productname_tokens_emb_flat = tf.reshape(query_productname_tokens_emb, [-1, nb_tokens_query_product_buckets],
                                                     name="query_productname_tokens_flat")
    query_productbrand_tokens_emb = tf.placeholder(tf.float32, [None, nb_tokens_query_product_buckets, 1],
                                                    name="query_productbrand_tokens")
    query_productbrand_tokens_emb_flat = tf.reshape(query_productbrand_tokens_emb, [-1, nb_tokens_query_product_buckets],
                                                     name="query_productbrand_tokens_flat")
    query_productname_brand_tokens_emb = tf.placeholder(tf.float32, [None, nb_tokens_query_product_buckets, 1],
                                                     name="query_productname_brand_tokens")
    query_productname_brand_tokens_emb_flat = tf.reshape(query_productname_brand_tokens_emb,
                                                      [-1, nb_tokens_query_product_buckets],
                                                      name="query_productname_brand_tokens_flat")
    ...
    query_product = tf.concat([query_productname_triplets_emb_flat,
                               ...
                               query_productname_tokens_emb_flat,
                               query_productbrand_tokens_emb_flat,
                               query_productname_brand_tokens_emb_flat
                               ], 1)
...
queries_set = loadDict(dictionry_file)
with tf.Session() as session:
    ...
    validation_query_productname_brand_triplets_batch_data, \
    validation_query_productname_tokens, \
    validation_query_productbrand_tokens_batch_data, \
    validation_query_productname_brand_tokens_batch_data = get_next_annotation_data(annotation_file, queries_set)
    ...
    fd_validation = {query_productname_triplets_emb: validation_query_productname_triplets,
             ...
             query_productname_tokens_emb: validation_query_productname_tokens,
             query_productbrand_tokens_emb: validation_query_productbrand_tokens_batch_data,
             query_productname_brand_tokens_emb: validation_query_productname_brand_tokens_batch_data,
             isTrain: False}
    ...
    for iteration_index in range(max_iterations):
        for batch_index in range(num_batches):
            ...
            if batch_index % 100 == 0:
                test_labels, \
                test_query_productname_triplets, \
                test_query_productbrand_triplets_batch_data , \
                test_query_productname_brand_triplets_batch_data, \
                test_query_productname_tokens, \
                test_query_productbrand_tokens_batch_data, \
                test_query_productname_brand_tokens_batch_data  = \
                next_batch(batch_positives*5,31,p_fptr, n_fptr,queries_set)
                ...
                fd_test = {y_true: test_labels, query_productname_triplets_emb: test_query_productname_triplets,
                           ...
                           query_productname_tokens_emb: test_query_productname_tokens,
                           query_productbrand_tokens_emb: test_query_productbrand_tokens_batch_data,
                           query_productname_brand_tokens_emb: test_query_productname_brand_tokens_batch_data,
                           isTrain: False}
                ...
                query_productname_tokens_batch_data, \
                query_productbrand_tokens_batch_data, \
                query_productname_brand_tokens_batch_data = \
                next_batch(batch_positives,negative_samples_factor,p_fptr, n_fptr, queries_set)
                ...
                session.run(adam_train_step, feed_dict={y_true: batch_labels,
                                                                      ...
                                                                      query_productname_brand_triplets_emb: query_productname_brand_triplets_batch_data,
                                                                        query_productname_tokens_emb: query_productname_tokens_batch_data,
                                                                        query_productbrand_tokens_emb: query_productbrand_tokens_batch_data,
                                                                        query_productname_brand_tokens_emb: query_productname_brand_tokens_batch_data,
                                                                      isTrain: True})
