...
max_positive_training_samples_size = 23885 - 885
...
brand_dict_test = {}
cateory_dict_test = {}
brand_dict_readindex_test = {}
cateory_dict_readindex_test = {}


batch_positives = 2


negative_samples_factor = 8
...
nb_triplets_query_product_buckets = 2**16
...
def loadBrandCategoryDictTest(inputtestfile):

    p_fptr = open(inputtestfile, mode=access_mode)
    for index in range(1,max_negative_training_samples_size-1):
        p_line = p_fptr.readline()
        ntokens = p_line.rstrip().split('|')
        if len(ntokens)<PRODUCTCATEGORY_INDEX_NEGATIVE:
            continue;
        if len(ntokens[PRODUCTBRAND_INDEX_NEGATIVE]) != 0:
            if ntokens[PRODUCTBRAND_INDEX_NEGATIVE] not in brand_dict_test:
                brand_dict_test[ntokens[PRODUCTBRAND_INDEX_NEGATIVE]]=[]
                brand_dict_readindex_test[ntokens[PRODUCTBRAND_INDEX_NEGATIVE]]=0
            if len(brand_dict_test[ntokens[PRODUCTBRAND_INDEX_NEGATIVE]])<100:
                brand_dict_test[ntokens[PRODUCTBRAND_INDEX_NEGATIVE]].append(p_line)


        if len(ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]) != 0:
            if ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE] not in cateory_dict_test:
                cateory_dict_test[ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]]=[]
                cateory_dict_readindex_test[ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]]=0
            if len(cateory_dict_test[ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]])<100:
                cateory_dict_test[ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]].append(p_line)
...
def loadBrandCategoryDict(inputfile):
    ...
    for index in range(1,max_negative_training_samples_size-1):
        ...
        if len(ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]) != 0:
            if ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE] not in cateory_dict:
...
def getNextCategorySampleTest(category):
    if category not in cateory_dict_test:
        return ""
    line =  cateory_dict_test[category][cateory_dict_readindex_test[category]]
    cateory_dict_readindex_test[category] = (cateory_dict_readindex_test[category]+1)%len(cateory_dict_test[category])
    return line



def getNextBrandSampleTest(brand):
    if brand not in brand_dict_test:
        return ""
    line =  brand_dict_test[brand][brand_dict_readindex_test[brand]]
    brand_dict_readindex_test[brand] = (brand_dict_readindex_test[brand]+1)%len(brand_dict_test[brand])
    return line
...
def get_next_batch():
    ...
    for index in range(batch_positives):
        ...
        #print("=========")
        #print("POS : "+positives_line)
        # print(positives_line)
        ptokens = positives_line.rstrip().split('|')

        if len(ptokens)<=PRODUCTCATEGORY_INDEX_POSITIVE:
            continue;
        ...
        while (negatives < negative_samples_factor):
            ...
            if negatives==negative_samples_factor :
                ...
                vtokens = negatives_line.rstrip().split('|')
                if len(vtokens) < PRODUCTCATEGORY_INDEX_NEGATIVE + 1:
                    continue
                if len(vtokens[PRODUCTCATEGORY_INDEX_NEGATIVE]) == 0:
                    continue
                if len(ptokens) < PRODUCTBRAND_INDEX_POSITIVE + 1:
                    continue
                if len(ptokens[PRODUCTBRAND_INDEX_POSITIVE]) == 0:
                    continue
                if vtokens[PRODUCTBRAND_INDEX_NEGATIVE] == ptokens[PRODUCTBRAND_INDEX_POSITIVE]:
                    continue
                ntokens=vtokens
                #trial_cat_sampling = 0
                #print(ntokens[PRODUCTBRAND_INDEX_NEGATIVE])
                #print(ptokens[PRODUCTBRAND_INDEX_POSITIVE])
                #print("NEG : "+negatives_line)
                #print(ntokens[PRODUCTBRAND_INDEX_NEGATIVE]+'\t'+ptokens[PRODUCTBRAND_INDEX_POSITIVE])
    ...
    return labels, query_productname_triplets, query_brand_triplets , query_productname_brand_triplets

           # query_productdescription_triplets, query_brand_triplets,\
...
def get_next_test_data(pos_test_file_name, neg_test_file_name, positives_count):
    ...
    for index in range(positives_count):
        ...
        while (negatives < test_negative_samples_factor):
            ...
            ntokens=""
            if  negatives == test_negative_samples_factor:
                #if trial_cat_sampling == 5:
                    # we have not been able to find a product of
                    # different brand in this category after 5 trials
                    #print("T "+str(product_index) + " " + str(negatives))
                ...
                if (len(category) == 0):
                    continue
                negatives_line = getNextCategorySampleTest(category)
                vtokens = negatives_line.rstrip().split('|')
                if len(vtokens) < PRODUCTCATEGORY_INDEX_NEGATIVE + 1:
                    continue
                if len(vtokens[PRODUCTCATEGORY_INDEX_NEGATIVE]) == 0:
            ...
            #     if (len(brand) == 0):
            #         continue
            #     negatives_line = getNextBrandSampleTest(brand)
            #     ntokens = negatives_line.rstrip().split('|')
            #     if len(ntokens) < PRODUCTCATEGORY_INDEX_NEGATIVE + 1:
            #         continue
            #     if len(ntokens[PRODUCTCATEGORY_INDEX_NEGATIVE]) == 0:
    ...
    return test_labels, query_productname_triplets,query_brand_triplets,query_productname_brand_triplets
            # \
           # query_productdescription_triplets, \
           # query_brand_triplets, \
           # query_productname_description_triplets, \
...
def getFeatures(query, productname, productbrand):
    ...
    result = np.append(query_productname_triplets,query_brand_triplets)
...
with tf.device('/cpu:0'):
    ...
    query_product = tf.concat([query_productname_triplets_emb_flat,
                               100*query_productbrand_triplets_emb_flat,
    ...
    cross_entropy = tf.reduce_mean(cross_entropy_tensor)
    adam_train_step = tf.train.AdamOptimizer(learning_rate=0.005).minimize(cross_entropy)
    # adam_train_step =  tf.train.FtrlOptimizer(
    #    learning_rate=0.1,
    #    l1_regularization_strength=1.0,
    #    l2_regularization_strength=1.0).minimize(cross_entropy)
    #metrics computation
...
loadBrandCategoryDictTest(test_negatives_file)
print('done!')
...
with tf.Session() as session:
    ...
    for iteration_index in range(max_iterations):
        ...
        for batch_index in range(num_batches):
            ...
            query_productname_brand_triplets_batch_data = get_next_batch()
            ...
            #     test_query_productname_brand_triplets_batch_data = get_next_test_data(test_positives_file,
            ...
            if batch_index % 1000 == 0:
                ...
                auc_val2=0
                test_labels, test_query_productname_triplets, test_query_productbrand_triplets_batch_data, \
                test_query_productname_brand_triplets_batch_data = get_next_validation_data(validation_file, 502)
                ...
                print('iteration ' + str(iteration_index) + ' auc@test  ' + str(auc_val2) + ' loss@test '+ str(loss)  +' done ' + str(end - start))
                start = time.time()
                ...
                auc_val2=0
                test_labels, test_query_productname_triplets, test_query_productbrand_triplets_batch_data, \
                test_query_productname_brand_triplets_batch_data = get_next_test_data(test_positives_file, test_negatives_file,10)
