...
with tf.device('/cpu:0'):
    ...
    optimize_critics = tf.train.RMSPropOptimizer(learning_rate=learning_rate_gans, name='discr_optimizer').minimize(
        cross_entropy_discr,
        var_list=variables_discr)

    optimize_generator = tf.train.RMSPropOptimizer(learning_rate=learning_rate_gans, name='gen_optimizer').minimize(
        generator_loss,
...
with tf.Session() as session:
    ...
    # for batch_index in range(300000):
    #     step += 1
    #     # Training on source
    ...
    #     #                                                      p_fptr, n_fptr)
    #
    #     batch_labels_source, query_triplets_batch_data, \
    #     productname_triplets_batch_data = get_next_test_data_embeddings(batch_positives, negative_samples_factor,
    #                                                          queries_source_embeddings_file,
    #                                                          product_source_positive_embeddings_file,
    #                                                         product_source_negative_embeddings_file)
    #
    #     batch_labels_source, query_triplets_batch_data_target, \
    #     productname_triplets_batch_data_target = get_next_test_data_embeddings(batch_positives, negative_samples_factor,
    #                                                          queries_target_embeddings_file,
    #                                                          product_target_positive_embeddings_file,
    #                                                         product_target_negative_embeddings_file)
    #     # batch_labels_target, query_productname_triplets_batch_data_target, \
    #     # query_triplets_batch_data_target, \
    #     # productname_triplets_batch_data_target = get_next_test_data(batch_positives, negative_samples_factor, fp_val,
    #     #                                                             fn_val)
    #
    #     feed_dict = {
    ...
    for iteration_index in range(max_iterations):
        ...
        for batch_index in range(num_batches):
            ...
            if batch_index % 10 == 0:
                ...
                test_labels, test_query_triplets, test_productname_triplets = \
                    get_next_test_data_embeddings(batch_positives * 5, 31, queries_target_embeddings_file,
                                          product_target_positive_embeddings_file,
                                          product_target_negative_embeddings_file)
                fd_test = {y_true: test_labels,
                           query_triplets_emb: test_query_triplets,
                ...
                #if batch_index % 1000 == 0:
                    # print(
                    #     'iteration source-target ' + str(iteration_index) + ' batch ' + str(batch_index + 1) + ' loss '
                ...
                #if batch_index % 1000 == 0:
                    #print('iteration source-target with projected products' + str(iteration_index) + ' batch ' + str(
                    #    batch_index + 1) + ' loss '
                    #      + str(validation_loss) + ' done ')
                ...
                if batch_index % 1000 == 0:
                    #print(
                    #    'iteration target-target ' + str(iteration_index) + ' batch ' + str(batch_index + 1) + ' loss '
                    #    + str(validation_loss) + ' done ')
                    print(
                        'iteration target-target ' + str(iteration_index) + ' batch ' + str(batch_index + 1) + ' loss '
                ...
                batch_labels, query_triplets_batch_data_target, productname_triplets_batch_data_target = \
                    get_next_test_data_embeddings(batch_positives, negative_samples_factor,
                                                  queries_target_embeddings_file,
                                                  product_target_positive_embeddings_file,
                                                  product_target_negative_embeddings_file)
                ...
                session.run([adam_train_step_dssm], feed_dict={y_true: batch_labels,
                                                                                ...
                                                                                productname_triplets_emb: productname_triplets_batch_data_target,
                                                                                isTrain: True,
                                                                                name_training_domain: 'target',
                                                                                isTargetProduct: False})

                # batch_labels_source, query_triplets_batch_data, \
                # productname_triplets_batch_data = get_next_test_data_embeddings(batch_positives,
                ...
                #
                # batch_labels_source, query_triplets_batch_data_target, \
                # productname_triplets_batch_data_target = get_next_test_data_embeddings(batch_positives,
                #                                                                        negative_samples_factor,
                #                                                                        queries_target_embeddings_file,
                #                                                                        product_target_positive_embeddings_file,
                #                                                                        product_target_negative_embeddings_file)
