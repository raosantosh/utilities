...
brand_max_length = 25
nb_triplets_query_product_buckets = 1<<16
...
def query2producttripletrepresentation(query, product, vocabulary):
    BUCKETS = vocabulary>>1
    features = Set()
    ...
    for gram in qgrams_4:
        if gram in pgrams_4:
            ...
            features.add( (abs(int(mmh3.hash(gram))) % BUCKETS) + BUCKETS)
    return features
...
with tf.Session(config=config) as session:
    ...
    batch_index = 1
    iterator_proto = tf.python_io.tf_record_iterator(path_data + "ngram_training_skus_formatted_diff_300K.proto")
    while True:
        step += 1

        serialized_example = next(iterator_proto, None)
        ...
        query_productname_index = np.array(serialized_batch.features.feature['query_productname'].int64_list.value)
        query_productname_triplets_batch_data = np.zeros((batch_size * nb_triplets_query_product_buckets))
        query_productname_triplets_batch_data[query_productname_index] = 1
        query_productname_triplets_batch_data = query_productname_triplets_batch_data.reshape(batch_size, nb_triplets_query_product_buckets, 1)

        query_productbrand_index = np.array(serialized_batch.features.feature['query_productbrand'].int64_list.value)
        query_productbrand_triplets_batch_data = np.zeros((batch_size * nb_triplets_query_product_buckets))
        query_productbrand_triplets_batch_data[query_productbrand_index] = 1
        query_productbrand_triplets_batch_data = query_productbrand_triplets_batch_data.reshape(batch_size, nb_triplets_query_product_buckets, 1)

        query_productname_brand_index = np.array(serialized_batch.features.feature['query_productname_brand'].int64_list.value)
        query_productname_brand_triplets_batch_data = np.zeros((batch_size * nb_triplets_query_product_buckets))
        query_productname_brand_triplets_batch_data[query_productname_brand_index] = 1
        query_productname_brand_triplets_batch_data = query_productname_brand_triplets_batch_data.reshape(batch_size, nb_triplets_query_product_buckets, 1)



        if batch_index % 500 == 0:

            fd_test = {y_true: batch_labels, query_productname_triplets_emb: query_productname_triplets_batch_data,
            ...
            if batch_index % 5000 == 0:
                ...
                precision, recall, specificity, TS = computeBestTS(y_score, validation_annotations, precision_memo,
                                                                   specificiy_memo)
