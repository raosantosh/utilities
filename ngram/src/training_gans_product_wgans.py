...
PRODUCTCAT_INDEX_POSITIVE = 4
PRODUCTCAT_INDEX_NEGATIVE = 3
...
def get_next_test_data(positives_count, test_negative_samples_factor, fpp, fnn):
    ...
    categories = np.zeros((test_data_size,number_of_categories))
    ...
    for index in range(positives_count):
        ...
        if (len(ptokens[PRODUCTCAT_INDEX_POSITIVE])) == 0:
            continue;
        cat_index = hash(ptokens[PRODUCTCAT_INDEX_POSITIVE])%number_of_categories
        categories[product_index,cat_index]=1
        product_index += 1
        negatives = 0
        ...
        while (negatives != test_negative_samples_factor):
            ...
            if (len(ptokens[PRODUCTCAT_INDEX_NEGATIVE])) == 0:
                continue;
            cat_index = hash(ptokens[PRODUCTCAT_INDEX_NEGATIVE]) % number_of_categories
            categories[product_index, cat_index] = 1
            product_index += 1
            negatives += 1
    ...
    return test_labels, query_productname_triplets, query_triplets, productname_triplets, categories
...
with tf.device('/cpu:0'):
    ...
    categories = tf.placeholder(tf.float32,[None, number_of_categories],name='categories')
    #GANS ON PRODUCTS!
    ...
    product_out_target_1 =  phi(product_target, n_output=second_layer_size,
                             ...
                             dropout=False,
                             reuse=tf.AUTO_REUSE)
    y_prediction_discriminator_target = phi(product_out_target_1, n_output=1,
                             droupout_rate=droupout_rate,
    ...
    product_out_source_1 =  phi(product_source, n_output=second_layer_size,
                             droupout_rate=droupout_rate,
                             ...
                             reuse=None,
                             orthogonal=True)
    y_prediction_discriminator_source = phi(product_out_source_1, n_output=1,
                             droupout_rate=droupout_rate,
                             activation=None, name='product_discriminator_l1',
                             isTraining=isTrain,
                             dropout=False,
...
with tf.Session() as session:
    ...
    for batch_index in range(30000000):
        ...
        productname_triplets_batch_data, categories_source = get_next_test_data(batch_positives, negative_samples_factor,
                                                             p_fptr, n_fptr)

        batch_labels_target, query_productname_triplets_batch_data_target, \
        query_triplets_batch_data_target, \
        productname_triplets_batch_data_target, categories_target = get_next_test_data(batch_positives, negative_samples_factor, fp_val, fn_val)
        ...
        if batch_index % 11 == 0:
            ...
            #if batch_index<25 and batch_index%5==0:
