...
nb_triplets_query_product_buckets = (1<<15)*3
...
QUERY_INDEX = 0
PRODCUTNAME_INDEX = 1
PRODUCTBRAND_INDEX = 2

#QUERY_INDEX = 2
#PRODCUTNAME_INDEX = 3
#PRODUCTBRAND_INDEX = 4
...
def query2producttripletrepresentation(query, product, vocabulary):
    ...
    for gram in qgrams_4:
        if gram in pgrams_4:
            ...
            pgrams_4.remove(gram)
        else:
            features.add( (abs(int(mmh3.hash(gram))) % BUCKETS) + BUCKETS)
    for gram in pgrams_4:
        features.add((abs(int(mmh3.hash(gram))) % BUCKETS) + (2*BUCKETS))
