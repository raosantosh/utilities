...
with tf.device('/gpu:0'):
    ...
    query_out_0 = tf.concat([query_triplets_emb_flat], 1)
    ...
    product_out_1 = tf.concat([productname_triplets_emb_flat], 1)
    ...
    product_target_non_zero_array = tf.not_equal(product_target, zero)
    product_target_indices = tf.where(product_target_non_zero_array)[:,1]
    histo_triplet_target_summary = tf.summary.histogram("histo triplet target", product_target_indices)



    product_out_target_1 =  multiphi(name_training_domain,product_target, n_output=first_layer_size,
                             droupout_rate=droupout_rate,
                             activation=None, name='generator_l0',
                             isTraining=isTrain,
                             dropout=False,
                             reuse=tf.AUTO_REUSE)
    product_out_target_2  =  multiphi(name_training_domain,product_out_target_1, n_output=second_layer_size,
                             droupout_rate=droupout_rate,
                             activation=None, name='generator_l1',
                             isTraining=isTrain,
                             dropout=False,
                             reuse=tf.AUTO_REUSE)
    y_prediction_discriminator_target = multiphi(name_training_domain,product_out_target_2, n_output=1,
                             droupout_rate=droupout_rate,
                             activation=None, name='product_discriminator_l2',
                             isTraining=isTrain,
                             dropout=False,reuse=tf.AUTO_REUSE)

    histo_target_summary =    tf.summary.histogram("histo generator target", product_out_target_2)
    ...
    product_source_non_zero_array = tf.not_equal(product_source, zero)
    product_source_indices = tf.where(product_source_non_zero_array)[:,1]
    histo_triplet_source_summary = tf.summary.histogram("histo source target", product_source_indices)

    product_out_source_1 =   multiphi(name_training_domain,product_source, n_output=first_layer_size,
                             droupout_rate=droupout_rate,
                             activation=None, name='generator_l0',
                             isTraining=isTrain,
                             dropout=False,
                             reuse=tf.AUTO_REUSE)
    product_out_source_2  =   multiphi(name_training_domain,product_out_source_1, n_output=second_layer_size,
                             droupout_rate=droupout_rate,
                             activation=None, name='generator_l1',
                             isTraining=isTrain,
                             dropout=False,
                             reuse=tf.AUTO_REUSE)
    y_prediction_discriminator_source =  multiphi(name_training_domain,product_out_source_2, n_output=1,
                             droupout_rate=droupout_rate,
                             activation=None, name='product_discriminator_l2',
                             isTraining=isTrain,
                             dropout=False,
    ...
    histo_source_summary = tf.summary.histogram("histo generator source", product_out_source_2)
    ...
    variables_projections = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "q_l")
    ...
    optimize_critics = tf.train.AdamOptimizer(learning_rate=learning_rate_gans, name='discr_optimizer').minimize(cross_entropy_discr,
                                                                                                               var_list=variables_discr)

    optimize_generator = tf.train.AdamOptimizer(learning_rate=learning_rate_gans, name='gen_optimizer').minimize(generator_loss,
                                                                                      var_list=variables_gen)

    histo_summaries = tf.summary.merge([histo_source_summary,histo_target_summary,histo_triplet_source_summary,histo_triplet_target_summary])
...
with tf.Session(config=config) as session:
    ...
    summary_writer_source_target_with_projection = tf.summary.FileWriter(log_location + '/source_target_with_projection',
                                                         session.graph)
    summary_writer_source_source_with_projection = tf.summary.FileWriter(
        log_location + '/source_source_with_projection',
        session.graph)
    ...
    while True:
        ...
        batch_labels_target = np.array(serialized_batch.features.feature['label'].int64_list.value)
        batch_labels_target = batch_labels_target.reshape(batch_size, 1)
        ...
        query_triplets_batch_data_target = query_triplets_batch_data_target.reshape(batch_size,
                                                                                    nb_triplets_query_product_buckets,
                                                                                    1)
        ...
        if batch_index % 10 == 0:
            ...
            fd_test[name_training_domain] = 'source_projection'
            val_loss_dssm, val_loss_dssm_summary = session.run(
                [cross_entropy_dssm, cross_entropy_dssm_summary],
                feed_dict=fd_test)
            summary_writer_source_source_with_projection.add_summary(val_loss_dssm_summary, step)

            fd_test = {y_true: batch_labels_target,
                       query_triplets_emb: query_triplets_batch_data_target,
                       productname_triplets_emb: productname_triplets_batch_data_target,
                       isTrain: False, name_training_domain: 'target',isTargetProduct: False}
            val_loss_dssm, val_loss_dssm_summary = session.run(
                [cross_entropy_dssm, cross_entropy_dssm_summary],
                feed_dict=fd_test)
            ...
            session.run([adam_train_step_dssm], feed_dict={y_true: batch_labels,
                                                                            ...
                                                                            productname_triplets_emb: productname_triplets_batch_data,
                                                                            isTrain: True,
                                                                            name_training_domain: 'source_projection', isTargetProduct: False})

            session.run([adam_train_step_dssm], feed_dict={y_true: batch_labels_target,
                                                                            query_triplets_emb: query_triplets_batch_data_target,
                                                                            productname_triplets_emb: productname_triplets_batch_data_target,
                                                                            isTrain: True,
            ...
            feed_dict = {
                ...
                isTrain: True,
                name_training_domain: 'source_projection'}
            if batch_index % 11 == 0:
                # gans validation

                feed_dict[isTrain] = False
                gen_loss_v, gen_summary, hsummaries  = session.run([generator_loss, generator_loss_summary,
                                                       histo_summaries], feed_dict)
                summary_writer_gans.add_summary(gen_summary, step)
                summary_writer_gans.add_summary(hsummaries, step)
