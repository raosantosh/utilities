...
from common.flip_gradient import flip_gradient
from common.experiment_db import update_experiment
...
parser.add_argument('--learning_rate_dans', type=float, default=0.00005, metavar='N')
...
parser.add_argument('--logs', type=int, default=-1, metavar='N')
...
update_experiment(metarun_id=args.metarun_id, run_id=args.run_id, metrics={"status": "started"})
...
def embedding(input_x, model_type, typ):
    """
    Embedding
    """
...
def dssm_atom(input_x, typ, domain, p_keep_for_dropout):
    ...
    with tf.variable_scope("dssm_{d}_{t}".format(d=domain, t=typ), reuse=tf.AUTO_REUSE):
        w1 = tf.get_variable(
            name='W1',
    ...
    return tf.nn.dropout(tf.matmul(x1_dropout, w2) + b2, keep_prob=p_keep_for_dropout)
...
with tf.device('/gpu:0'):
    ...
    # Dropout rate
    p_keep_for_dropout = tf.placeholder(tf.float32, shape=(), name="dropout_rate")

    # Placeholders for product and query (triplets)
    product_input = tf.placeholder(tf.float32, [None, nb_triplets_query_product_buckets, 1],
                                   name="productname_triplets_dssm")
    query_input = tf.placeholder(tf.float32, [None, nb_triplets_query_product_buckets, 1],
                                 name="query_triplets")
    ...
    embedded_product_source = embedding(product, model_type="source", typ="product")
    embedded_product_dans = embedding(product, model_type="source_dans", typ="product")
    embedded_product_target = embedding(product, model_type="target", typ="product")

    embedded_query_source = embedding(query, model_type="source", typ="query")
    embedded_query_dans = embedding(query, model_type="source_dans", typ="query")
    embedded_query_target = embedding(query, model_type="target", typ="query")

    query_after_dssm_atom = tf.cond(pred=tf.equal(model_type, tf.constant('source')),
                                    true_fn=lambda: dssm_atom(embedded_query_source, typ="query",
                                                              domain="source", p_keep_for_dropout=p_keep_for_dropout),
    ...
    product_after_dssm_atom = tf.cond(pred=tf.equal(model_type, tf.constant('source')),
    ...
    cross_entropy_dssm = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(labels=y_task, logits=y_prediction_dssm), name="cross_entropy_dssm")

    dssm_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "dssm")
    logger.info("Variables for DSSM are {}".format(dssm_variables))

    embedding_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "embedding")
    ...
    discriminator_loss = tf.reduce_sum(tf.losses.log_loss(predictions=y_gans_pred, labels=y_gans))
    ...
    logger.info("Variables for discriminator are {}".format(discriminator_variables))

    adam_train_step_discriminator = tf.train.AdamOptimizer(learning_rate=args.learning_rate_dans).minimize(
        discriminator_loss, var_list=discriminator_variables)

    # Computing accuracy
    is_correct_discriminator = tf.equal(tf.sign(y_gans - 0.5), tf.sign(y_gans_pred - 0.5))
    accuracy_discriminator = tf.reduce_mean(tf.cast(is_correct_discriminator, tf.float32))
...
if args.max_nb_processes > 1:
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.9 / args.max_nb_processes)
else:
...
with tf.Session(config=config) as session:
    ...
    print("Using log location for tensorboard {} (run_id is {}, metarun_id is {})".format(log_location, args.run_id,
                                                                                          args.metarun_id))
    try:
    ...
    def get_writer(mode):
        return tf.summary.FileWriter(
            log_location + '/{}_{}_{}'.format(args.metarun_id, args.run_id, mode),
            session.graph)
    ...
    while keep_iterating:
        ...
        for model_typ in ["source", "target"]:
            ...
            if epochs[model_typ] <= args.max_epoch_for_training:
                serialized_data, is_new_epoch = iterators["train"][model_typ].get_next_serialized_batch()
                q, p, y = parse_serialized_batch(serialized_data=serialized_data, batch_size=batch_size)

                if is_new_epoch:
                    logger.info("Finished epoch {} for {}".format(epochs[model_typ], model_typ))
                ...
                if step % 100 == 0:
                    summary_writers_train[model_typ].add_summary(val_summary_dssm, step)

        ########################
        # Training source_dans #
        ########################

        if min(epochs["dans_target"], epochs["dans_source"]) <= args.max_epoch_for_training:
            model_typ = "source_dans"
            serialized_data, is_new_epoch_source = iterators["train"]["dans_source"].get_next_serialized_batch()
            q_source, p_source, y_source = parse_serialized_batch(serialized_data=serialized_data, batch_size=batch_size)

            serialized_data, is_new_epoch_target = iterators["train"]["dans_target"].get_next_serialized_batch()
            _, p_target, _ = parse_serialized_batch(serialized_data=serialized_data, batch_size=batch_size)

            if is_new_epoch_source:
                logger.info("Finished epoch {} for dans_source".format(epochs["dans_source"]))
                epochs["dans_source"] = epochs["dans_source"] + 1

            if is_new_epoch_target:
                logger.info("Finished epoch {} for dans_target".format(epochs["dans_target"]))
            ...
            labels_for_dans = np.concatenate([np.zeros((32, 1)), np.ones((32, 1))], axis=0)
            ...
            val_y_real, val_y_pred = session.run([y_gans, y_gans_pred], feed_dict=feed_dict_dans)

            val_summary_discr, _ = session.run([summary_discriminator, adam_train_step_discriminator],
                                               feed_dict=feed_dict_dans)

            if step % 100 == 0:
                summary_writers_train[model_typ].add_summary(val_summary_discr, step)
        ...
        if step % 100 == 0:
            serialized_data, _ = iterators["test"]["target"].get_next_serialized_batch()
            q_target, p_target, y_target = parse_serialized_batch(serialized_data=serialized_data,
            ...
            for model_typ in ["source", "target", "source_dans"]:
                ...
                summary_writers_test[model_typ].add_summary(val_summary_dssm, step)
    ...
    for validation_domain in ["source", "target"]:
        ...
        while keep_iterating:
            ...
            serialized_data, is_new_epoch = iterators["validation"][validation_domain].get_next_serialized_batch()
            q_target, p_target, y_target = parse_serialized_batch(serialized_data=serialized_data,
                                                                  batch_size=batch_size_test)
            ...
            for model_typ in ["source", "target", "source_dans"]:
                ...
                xentropy, _ = session.run([cross_entropy_dssm, dssm_auc_update_op], feed_dict=feed_dict)

                final_metrics[model_typ]["xentropy_{}".format(validation_domain)] =\
                    final_metrics[model_typ].get("xentropy_{}".format(validation_domain), 0) + xentropy
        ...
        for model_typ in ["source", "target", "source_dans"]:
            ...
            final_metrics[model_typ]["auc_{}".format(validation_domain)] = np.float64(final_auc)

            # Normalizing cross-entropy
            final_metrics[model_typ]["xentropy_{}".format(validation_domain)] =\
                np.float64(final_metrics[model_typ]["xentropy_{}".format(validation_domain)] / step)
    ...
    update_experiment(metarun_id=args.metarun_id, run_id=args.run_id, metrics=final_metrics)
    logger.info("Updating done")
